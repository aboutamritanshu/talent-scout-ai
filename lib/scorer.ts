...
function synonymsOf(skill: string): string[] {
...
function skillMatches(target: string, candidateSkills: string[]): boolean {
  ...
  for (const token of targetTokens) {
    ...
    for (const cand of normCandidate) {
      ...
      if (candBase === tokenBase) return true;
...
function evalSkills(jd: ParsedJD, candidate: Candidate): SkillsEval {
  ...
  for (const skill of required) {
    if (skillMatches(skill, candidate.skills)) matched.push(skill);
    else missing.push(skill);
  }
  const niceMatched: string[] = [];
  for (const skill of nice) {
    if (skillMatches(skill, candidate.skills)) niceMatched.push(skill);
  }
  ...
  if (required.length === 0 && nice.length === 0) {
    ...
    if (nice.length > 0) {
      parts.push(
        `${niceMatched.length} of ${nice.length} nice-to-have`,
      );
    }
    note = parts.join("; ") + ".";
  }
...
function evalExperience(jd: ParsedJD, candidate: Candidate): CategoryEval {
...
function evalDomain(jd: ParsedJD, candidate: Candidate): CategoryEval {
...
type DegreeLevel = 0 | 1 | 2 | 3 | 4; // 0 = unknown, 1 diploma, 2 bachelor, 3 master, 4 phd/professional

const DEGREE_KEYWORDS: { level: DegreeLevel; match: RegExp }[] = [
...
function detectDegreeLevel(text: string): DegreeLevel {
  ...
  for (const { level, match } of DEGREE_KEYWORDS) {
    if (match.test(text)) return level;
  }
  return 0;
}

function evalEducation(jd: ParsedJD, candidate: Candidate): CategoryEval {
...
function summarizeStrengthsAndGaps(
  ...
  for (const row of breakdown) {
    if (row.score >= 85) strengths.push(`${row.label}: ${row.note}`);
    else if (row.score < 60) gaps.push(`${row.label}: ${row.note}`);
  }

  if (skills.niceMatched.length > 0) {
    strengths.push(
      `Bonus skills present: ${skills.niceMatched.join(", ")}.`,
    );
  }
